...
for spikeTime in neuron1:
	...
	if(x[index] > 195 and y[index] > 140):
		neuron1_Right_Up.append(spikeTime)
	elif(x[index] < 145 and y[index] > 140):
		neuron1_Left_Up.append(spikeTime)
	elif(x[index] > 195 and y[index] < 80):
		neuron1_Right_Down.append(spikeTime)
	elif(x[index] < 145 and y[index] < 80):
		neuron1_Left_Down.append(spikeTime)
	else:
		neuron1_Center.append(spikeTime)


for i in range(0,len(time)):
	time[i] = int(round(time[i]/10000))

for i in range(0,len(neuron1_Right_Up)):
	neuron1_Right_Up[i] = int(round(neuron1_Right_Up[i]/10000))
for i in range(0,len(neuron1_Left_Up)):
	neuron1_Left_Up[i] = int(round(neuron1_Left_Up[i]/10000))
for i in range(0,len(neuron1_Right_Down)):
	neuron1_Right_Down[i] = int(round(neuron1_Right_Down[i]/10000))
for i in range(0,len(neuron1_Left_Down)):
	neuron1_Left_Down[i] = int(round(neuron1_Left_Down[i]/10000))
for i in range(0,len(neuron1_Center)):
	neuron1_Center[i] = int(round(neuron1_Center[i]/10000))


counter_RU=collections.Counter(neuron1_Right_Up)
counter_LU=collections.Counter(neuron1_Left_Up)
counter_RD=collections.Counter(neuron1_Right_Down)
counter_LD=collections.Counter(neuron1_Left_Down)
counter_C=collections.Counter(neuron1_Center)

for i in range(time[0],time[len(time)-1]):
	modifiedTime.append(i)
	if i in neuron1_Right_Up:
		countList1.append(counter_RU[i])
	else:
		countList1.append(0)
	if i in neuron1_Left_Up:
		countList2.append(counter_LU[i])
	else:
		countList2.append(0)
	if i in neuron1_Right_Down:
		countList3.append(counter_RD[i])
	else:
		countList3.append(0)
	if i in neuron1_Left_Down:
		countList4.append(counter_LD[i])
	else:
		countList4.append(0)
	if i in neuron1_Center:
...
plt.suptitle("Firing rates of neuron 1 in different part of the maze")
plt.show()
